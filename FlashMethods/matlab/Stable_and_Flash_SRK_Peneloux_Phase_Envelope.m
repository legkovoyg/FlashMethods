...
Ttest=673.15:5:673.15; %100:10:200;
Ptest=0.1:1:0.1;
Ptest=10:1:10;

%Ptest = [48.0 46 43 40  36 32 28 24 20 14 7 0.48]
...
Sm = sum(x_i.*(fz_i./fl_i));
x_i = (x_i.*((fz_i./fl_i)))/Sm;
Pold = P;
...
if (sign==1)
    ...
    if ((abs(P-Pold)<0.0001) && (max(abs(1-fz_i./fl_i))<0.0001))
        %fprintf('Количество итераций !!! \n');
        m;
        P;
        break
    end
...
%     (Aw/Bw)*(Biw+Ciw)/(Z_v + Bw + Cw ) + (Aw/Bw)*Ciw/(Z_v + Cw ));
% 
% Sm = sum(y_i.*(fw_i./fl_i).^-1);
% y_i = (y_i.*((fw_i./fl_i).^-1))/Sm;
% P
% 
% Sm
% Pold = P;
...
% % if (step==1)
% % 
% %     Nmol_mix = zeros(1,size(Ptest,2));
% %     Nmol_l = zeros(1,size(Ptest,2));
% %     Nmol_v = zeros(1,size(Ptest,2));
% %     Vl_Vmix = zeros(1,size(Ptest,2));
% %     Vol_l = zeros(1,size(Ptest,2));
% %     Vol_v = zeros(1,size(Ptest,2));
% %     z_CVD = zeros(N,size(Ptest,2));
% %     y_i_CVD = zeros(N,size(Ptest,2));
% %     x_i_CVD = zeros(N,size(Ptest,2));
% % 
% %     % Необходимо правильно рассчитать Vol_mix0 !!!!
% %     Nmol_mix(step) = 1;
% %     Vol_mix0 = 82.49;
% % else
% %     Nmol_l(step) = Nmol_mix(step-1)*(1-W);
% %     Vol_l(step) = Nmol_l(step)*molweight_l/rho_l;
% %     %1000*Vol_l / 82.49;
% %     Vol_v(step) = Vol_mix0 - 1000*Vol_l(step);
% %     Nmol_v(step) = Vol_v(step)*rho_v/(1000*molweight_v); 
% %     Nmol_mix(step) = Nmol_v(step) + Nmol_l(step);
% % 
% %     z_CVD(:,step) = ((Nmol_v(step)*y_i + Nmol_l(step)*x_i)/Nmol_mix(step))';
% %     y_i_CVD(:,step) = y_i';
% %     x_i_CVD(:,step) = x_i';
% %     Vl_Vmix(step) = 1000*Vol_l(step)/Vol_mix0;
% %     z = (Nmol_v(step)*y_i + Nmol_l(step)*x_i)/Nmol_mix(step);
% % end
% 
% 
% %% Расчёт CME
...
% fprintf(fid2,'%3.5f %3.9f \t \n', P, m);
% 
% fprintf(fidH,'%3.5f %5.5f %5.5f %3.5f %3.5f %3.5f %3.5f %3.5f %3.5f %3.5f %3.5f\t \n', P, H, Hl, Cp, Cpl, Cv, Cvl, S, Sl, JTh, JThl);
