...
class SRKFlash(base_flash.Flash):
    ...
    def calculate(self) -> equation_dto.EquationResultDTO:
        ...
        ac_i = 0.42748 * self._R**2 * self._Tkr**2.0 / self._Pkr
        ...
        fz_i, self._Z_init = self._calculate(self._p, self._t, self._z, b_i, c_a_i, 1)
        m = 0
        indx = 0
        eps_f = 1
        Ri_v = 1
        ...
        while m < self._MaxIterationStable:
            ...
            fw_i, Z_v = self._calculate(self._p, self._t, self._y_i, b_i, c_a_i, 1)
            Ri = fz_i / (Sv * fw_i)
            Ri_v = np.sum((Ri - 1) ** 2)
        ...
        fz_i, Z_v = self._calculate(self._p, self._t, self._z, b_i, c_a_i, 1)
        ml = 0
        Ri_l = 1
        ...
        while ml < self._MaxIterationStable:
            ...
            fl_i, Z_l = self._calculate(self._p, self._t, self._x_i, b_i, c_a_i, 0)
            Ri = Sl * fl_i / fz_i
            Ri_l = np.sum((Ri - 1) ** 2)
